...
class FigmaAuthType(str, Enum):
    """Authentication types supported by the Figma connector."""
...
class FigmaResponse(BaseModel):
    """Standardized Figma API response wrapper.
...
class FigmaRESTClientViaOAuth(HTTPClient):
    """Figma REST client via OAuth 2.0 access token.
...
class FigmaTokenConfig(BaseModel):
    """Configuration for Figma client via Personal Access Token.
    ...
    def create_client(self) -> FigmaRESTClientViaToken:
        return FigmaRESTClientViaToken(self.token)


class FigmaOAuthConfig(BaseModel):
    """Configuration for Figma client via OAuth 2.0.
    ...
    def create_client(self) -> FigmaRESTClientViaOAuth:
        return FigmaRESTClientViaOAuth(
            self.access_token,
            self.client_id,
            self.client_secret,
        )
...
class FigmaAuthConfig(BaseModel):
    """Auth section of the Figma connector configuration from etcd."""

    authType: FigmaAuthType = FigmaAuthType.TOKEN
...
class FigmaCredentialsConfig(BaseModel):
    """Credentials section of the Figma connector configuration."""
...
class FigmaConnectorConfig(BaseModel):
    """Top-level Figma connector configuration from etcd."""

    auth: FigmaAuthConfig = Field(default_factory=FigmaAuthConfig)
    credentials: FigmaCredentialsConfig = Field(
        default_factory=FigmaCredentialsConfig
    )
...
class FigmaClient(IClient):
    ...
    def __init__(
        self,
        client: FigmaRESTClientViaToken | FigmaRESTClientViaOAuth,
    ) -> None:
        """Initialize with a Figma client object."""
        super().__init__()
        self.client = client
    ...
    def get_client(
        self,
    ) -> FigmaRESTClientViaToken | FigmaRESTClientViaOAuth:
        """Return the Figma client object."""
        return self.client
    ...
    def build_with_config(
        cls,
        config: FigmaTokenConfig | FigmaOAuthConfig,
    ) -> "FigmaClient":
        """Build FigmaClient with configuration.

        Args:
            config: FigmaTokenConfig or FigmaOAuthConfig instance
    ...
    async def build_from_services(
        ...
        """Build FigmaClient using configuration service.

        Supports two authentication strategies:
        1. TOKEN: For personal access tokens
        ...
        try:
            ...
            if not raw_config:
                raise ValueError("Failed to get Figma connector configuration")

            connector_config = FigmaConnectorConfig.model_validate(raw_config)

            if connector_config.auth.authType == FigmaAuthType.OAUTH:
            ...
            elif connector_config.auth.authType == FigmaAuthType.TOKEN:
                token = (
                ...
                token_config = FigmaTokenConfig(token=token)
            ...
            logger.error(
                f"Failed to build Figma client from services: {str(e)}"
            )
            raise
    ...
    async def build_from_toolset(
        ...
        try:
            ...
            logger.error(
                f"Failed to build Figma client from toolset: {str(e)}"
            )
            raise
    ...
    async def _get_connector_config(
        ...
        """Fetch connector config from etcd for Figma."""
        try:
            ...
            if not raw:
                raise ValueError(
                    f"Failed to get Figma connector configuration "
                    f"for instance {connector_instance_id}"
                )
            return cast(dict[str, Any], raw)
        except Exception as e:
            logger.error(f"Failed to get Figma connector config: {e}")
            raise ValueError(
                f"Failed to get Figma connector configuration "
                f"for instance {connector_instance_id}"
            ) from e
